...
class MaxCutVisualization(Visualization):
    ...
    def visualize(
        ...
        edgeColors = [
            w.get("weight") for (u, v, w) in problem_instance_graph.edges(data=True)
        ]
        nodes = nx.draw_networkx_nodes(
            problem_instance_graph,
            pos,
            node_color=colors,
            node_size=node_size,
            alpha=1,
        ...
        label_dict = {
            i: list(range(problem_instance_graph.number_of_nodes()))[i]
            for i in range(
                0, len(list(range(problem_instance_graph.number_of_nodes())))
            )
        }
...
class TspVisualization(Visualization):
    class TSPVisualization:
        ...
        def draw_solution(
            ...
            plt.ylim(ylim)
            plt.scatter(
                [x for x, _ in self.instance],
                [y for _, y in self.instance],
                color="black",
                s=dot_size,
            )
                ...
                for p1, p2 in product(self.instance, self.instance):
                    if not np.array_equal(p1, p2):
                        plt.plot(
                            (p1[0], p2[0]),
                            (p1[1], p2[1]),
                            color=full_graph_color,
                            alpha=full_graph_alpha,
            ...
            for path, color, line_width in self.solutions:
                for p1, p2 in zip(path[:-1], path[1:]):
                    plt.plot(
                        (self.instance[p1][0], self.instance[p2][0]),
                        (self.instance[p1][1], self.instance[p2][1]),
                        color=color,
